if (
    ...
    var OptiMonkRegistry = {
        account: 93950,
        jfJsScriptUrl: '//gs-cdn.optimonk.com/jfclientsdk/latest/jfclientsdk.min.js',
    ...
    var OptiMonk = (function(OptiMonk){
        ...
        try {
            ...
            if (wnd) {
               OptiMonk.native.XMLHttpRequest = wnd.XMLHttpRequest;
               OptiMonk.native.fetch = wnd.fetch;
               OptiMonk.native.Promise = wnd.Promise;
               OptiMonk.native.Date = wnd.Date;
            }
        } catch (err) {}
        ...
        OptiMonk.sendLoadRequest = function () {
            ...
            body = body + '&abTestCookies=' + OptiMonk.Util.stringify(OptiMonk.getAbTestCookies())

            var loadUrl = OptiMonkRegistry.baseUrl + '/public/' + OptiMonkRegistry.account + '/js/load'

            if (false) {
              loadUrl += '?t=' + (new Date()).getTime()
            }
        ...
        OptiMonk.getSelectedVariantId = function () {
          if (window.ShopifyAnalytics) {
              var addToCartForm = document.querySelector('form[action^="/cart/add"][data-delm-is-primary-form="true"]')
                  ? document.querySelector('form[action^="/cart/add"][data-delm-is-primary-form="true"]')
                  : document.querySelector('form[action^="/cart/add"]');

              if (addToCartForm) {
                var variantSelect = addToCartForm.querySelector('[name="id"]');
              ...
              return OptiMonk._get(ShopifyAnalytics, 'meta.selectedVariantId') || null
          } else if (window.ShopRenter) {
              return OptiMonk._get(ShopRenter, 'product.id') || null
          }

          return null
        }

        OptiMonk.getViewedProductIds = function () {
            var productId = null;
            var variantId = OptiMonk.getSelectedVariantId();

            if (window.ShopifyAnalytics) {
                productId = OptiMonk._get(ShopifyAnalytics, 'meta.product.id')

                var variants = OptiMonk._get(ShopifyAnalytics, 'meta.product.variants')
                if (variants) {
    ...
    (function (OptiMonk) {
        function registerProductChangeListeners (ids) {
            var lastIds = { productId: ids.productId, variantId: ids.variantId }
            if (window.ShopifyAnalytics) {
                setInterval(function () {
                    var currentIds = OptiMonk.getViewedProductIds()
                    if (currentIds && currentIds.variantId && lastIds.variantId !== currentIds.variantId) {
                        ...
                        JFClientSDK.v1.registerProductView({ product: currentIds }).go()
                        console.log("Detected Shopify variant change.", lastIds)
                    }
                }, 1000)
            }
        }
        function logEmailServiceProvider () {
            try {
                ...
                if (params.has('_ke')) {
                    provider = 'klaviyo';
                } else if (params.has('mc_cid') || params.has('mc_eid')) {
                    provider = 'mailchimp';
                } else if (params.has('__s')) {
                    provider = 'drip';
                } else if (params.has('utm_source')) {
                    var utmSource = params.get('utm_source')
                    if (utmSource === 'ActiveCampaign') {
                      provider = 'activecampaign'
                    }
                }

                JFClientSDK.v2.registerCustomEvent('espPageView', {
                    provider,
                    ...extra,
                });
            } catch (err) {
                console.log('unable to log email service provider:', err.message)
            }
        }
        function logProductView () {
            var ids = OptiMonk.getViewedProductIds()
            if (ids) {
                registerProductChangeListeners(ids)
                return JFClientSDK.v1.registerProductView({ product: ids }).go()
            }
        ...
        function logCheckout () {
            if (Shopify.checkout) {
                JFClientSDK.v2.registerCustomEvent('eoo', {
                    orderId: Shopify.checkout.order_id.toString(),
                    total: parseFloat(Shopify.checkout.total_price),
        ...
        function logShoprenterCheckout () {
            if (ShopRenter.lastOrder) {
                JFClientSDK.v2.registerCustomEvent('eoo', {
                    orderId: ShopRenter.lastOrder.id,
                    total: ShopRenter.lastOrder.total,
        ...
        try {
            ...
            if (window.ShopRenter || window.Shopify || OptiMonkRegistry.account === 58417 || OptiMonkRegistry.account === 88202) {
                OptiMonk.addScript('jetfabric', OptiMonkRegistry.jfJsScriptUrl)
            }

            var WEB_SELECTOR_HEADER_NAME = 'x-om-web-selector-token'
            var params = new URLSearchParams(location.search)
            var webSelectorTokenInUrl = params.get(WEB_SELECTOR_HEADER_NAME)
            var webSelectorCookiePresent = localStorage.getItem(WEB_SELECTOR_HEADER_NAME)
            var webSelectorEnabled = OptiMonkRegistry.pncJsScriptUrl && (webSelectorTokenInUrl || webSelectorCookiePresent)
            if (webSelectorEnabled) {
                OptiMonk.addScript('webSelector', OptiMonkRegistry.pncJsScriptUrl + '?t=' + (new Date()).getTime())
            }

            var disabledByExperiment = false
            var cohortToAssignTo = ''
            var experiment = OptiMonkRegistry.experiment
            var cohortsCookieKey = 'om-experiment-cohorts'
            var cohorts = []
            try {
              var fv = OptiMonk._get(OptiMonk.getCookie(), `${OptiMonkRegistry.account}.fv`)
              if (experiment && (!fv || fv >= 1610549143)) {
                  var cohortsCookie = OptiMonk.Cookie.local.getItem(cohortsCookieKey)
                  var splitA = `${experiment.name}-${experiment.version}-splitA`
                  var splitB = `${experiment.name}-${experiment.version}-splitB`
                  if (cohortsCookie) {
                      cohorts = JSON.parse(cohortsCookie)
                  }

                  if (cohorts.indexOf(splitA) !== -1) {
                      disabledByExperiment = true
                  } else if (cohorts.indexOf(splitB) === -1) {
                      if (Math.floor(Math.random() * 100) + 1 <= experiment.splitA) {
                          cohortToAssignTo = splitA
                          cohorts.push(splitA)
                          disabledByExperiment = true
                      } else {
                          cohortToAssignTo = splitB
                          cohorts.push(splitB)
                      }
                  }
                  OptiMonk.Cookie.local.setItem(cohortsCookieKey, JSON.stringify(cohorts))
                  OptiMonkRegistry.cohorts = cohorts.join(',')
              }
            } catch (err) {
              console.log('Failed to initialize experiments:', err.message)
              console.log(err)
            }
            ...
            document.addEventListener('jetfabricLoaded', function () {
                ...
                if (window.Shopify) {
                    ...
                    storeProvider = 'shoprenter'
                    shopId = window.ShopRenter.shop.name
                    customerId = OptiMonk._get(ShopRenter, 'customer.userId')
                } else if (OptiMonkRegistry.account === 58417) {
                  shopId = 'blendjet.myshopify.com'
                  storeProvider = 'shopify'
                } else if (OptiMonkRegistry.account === 88202) {
                  shopId = 'rugby-is-life.myshopify.com'
                  storeProvider = 'shopify'
                }
                if (!customerId) {
                ...
                if (shopId) {
                    ...
                    JFClientSDK.v2
                        ...
                        .registerCustomerAddress({
                            ...
                            customerAddress: OptiMonkRegistry.clientId,
                            addressType: 'deviceId'
                        })
                        .registerCustomerAddress({
                            provider: storeProvider,
                            providerServiceId: shopId,
                            customerAddress: customerId || null,
                            addressType: 'customerId'
                        })

                    if (cohortToAssignTo) {
                        JFClientSDK.v2.registerCustomEvent('cohortAssign', {
                            cohort: cohortToAssignTo
                        })
                    }
                    logProductView()

                    var isCheckoutPage = window.Shopify && window.Shopify.Checkout && window.Shopify.Checkout.page === 'thank_you'
                    if (isCheckoutPage) {
                        logCheckout()
                    }

                    var isShoprenterCheckoutPage = window.ShopRenter && window.ShopRenter.page && window.ShopRenter.page.route === 'checkout/success'
            ...
            OptiMonk.Data.load()
                .then(function () {
                    if (!webSelectorEnabled && !disabledByExperiment && OptiMonk.shouldContinueLoading()) {
                        OptiMonk.sendLoadRequest();
                    }
                });
            // Logging
            if (OptiMonkRegistry.account === 30818 || OptiMonkRegistry.account === 58417) {
                OptiMonk.addListener(window, 'error', ErrorReporter.sendEventError)
            }
            // BlendJet custom cart code
            if (OptiMonkRegistry.account === 58417) {
                setInterval(function() {
                    var jQuery = OptiMonk.$
                    if (!OptiMonk.Visitor || !jQuery) {
                        return
                    }
                    var adapter = OptiMonk.Visitor.createAdapter();

                    adapter.Cart.clear();
                    var cartItems = OptiMonk._get($nuxt, '$store.state.cart.lineItems') || []
                    jQuery.each(cartItems, function (i, item) {
                        ...
                        adapter.Cart.add(
                            item.id,
                            {
                                ...
                                line_price: parseFloat(variant.compareAtPrice),
                                product_id: item.id,
